...
:r E:\cohd\ehr_prevalence\load_iatrogenic_tsql.sql

-- Export person ID, start date, and concept IDs for conditions, drugs, and procedures
:OUT C:\Users\ct2865\Documents\cohd_data\data\unique_patient_concept_pairs_date_2.txt
...
:OUT C:\Users\ct2865\Documents\cohd_data\data\person.txt
...
FROM
	...
	WHERE c.domain_id = 'Condition'
		AND c.vocabulary_id IN ('SNOMED', 'MedDRA'))) y
;
...
FROM
	...
	JOIN concept c ON ca.ancestor_concept_id = c.concept_id
	WHERE c.domain_id = 'Drug'
		AND c.vocabulary_id IN ('RxNorm', 'ATC'))) y
;
...
FROM
	...
	JOIN concept c ON ca.ancestor_concept_id = c.concept_id
	WHERE c.domain_id = 'Procedure'
		AND c.vocabulary_id IN ('SNOMED', 'MedDRA', 'ICD10PCS'))) y
;
...
:OUT C:\Users\ct2865\Documents\cohd_data\data\concept_descendants_direct_TEST.txt
...
-- Note 2: Using variables to store the vocabs instead of using a subquery because the 
-- subquery was extremely slow despite being non-correlated. 
:OUT C:\Users\ct2865\Documents\cohd_data\data\concept_descendants_all_observed_TEST.txt
SELECT concept_id, descendant_concept_id
FROM
	((SELECT ca.ancestor_concept_id AS concept_id, ca.descendant_concept_id
	FROM #hierarchical_condition_concepts hc
...
:OUT C:\Users\ct2865\Documents\cohd_data\data\concepts.txt
